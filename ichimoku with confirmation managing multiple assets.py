...
ASSETS = ['ETHUSDT', 'BTCUSDT', 'BNBUSDT', 'XRPUSDT', 'LTCUSDT']  # Example assets for multiple trading
# --- Logging Setup ---
...
class ForwardIchimokuTrader:
    def __init__(self, symbol: str, interval: str, lookback: int,
        ...
        self.interval_5m = Client.KLINE_INTERVAL_5MINUTE  # Added for 5-minute timeframe confirmation
    ...
    def _get_server_time(self):
        """Fetch the current server time from Binance API."""
        try:
            ...
            return datetime.utcnow()  # Fallback to local UTC time if API call fails
    ...
    def _fetch_initial_data(self) -> bool:
        ...
        try:
            ...
            print(self.df.tail())#for dubugging purpose only no use at all
    ...
    def _fetch_latest_candle(self) -> bool:
        ...
        try:
            ...
            latest_kline = klines[-1]  # Fixed: Use the most recent candle
    ...
    def _check_signals(self) -> tuple[bool, bool]:
        ...
        required_cols = ['Close', 'conversion line', 'base line']
        ...
        buy_signal = (current_close > close_t_minus_26) and (conversion_line > base_line)
        sell_signal = (current_close < close_t_minus_26) and (conversion_line < base_line)
    ...
    def _check_5m_confirmation(self, direction: str) -> bool:
        ...
        if direction == 'buy':
            ...
            self.logger.debug(f"Invalid direction {direction} in _check_5m_confirmation.")
    ...
    def _manage_position(self) -> None:
        ...
        if self.position == 1:
            ...
            self.logger.debug(f'new sl_level{self.sl_level}')
    ...
    def _check_accidental_exit(self):
        ...
        if self.position == 1:
            ...
            self.logger.info("No position to check for accidental exit.")
    ...
    def run(self) -> None:
        for asset in ASSETS:
            self.symbol = asset  # Update the symbol for the current asset
            self.logger.info(f"Trading asset: {self.symbol}")
            
            if not self._fetch_initial_data():
                self.logger.error("Failed to fetch initial data. Stopping asset iteration.")
                continue

            if not self._calculate_indicators():
                self.logger.warning("Initial indicator calculation failed. Will retry on next candle.")
            else:
                self._manage_position()  # Check for immediate trade entry after historical data
            ...
            while True:
                ...
                self.logger.info("Finished trading for asset: " + self.symbol)
...
if __name__ == "__main__":
    if not API_KEY or not API_SECRET:
        ...
        # Option 1: Run each trader sequentially (one after the other)
        for trader in traders:
            trader.run()
        
        # Option 2: Use threading or async to run them concurrently
