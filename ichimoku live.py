...
class ForwardIchimokuTrader:
    ...
    def accidental_exit(self):
        ...
        if self.position == 1:  # Long position
            if not(not pd.isna(psar_long) and 
                ...
                # Reset position state after canceling orders
                self._reset_position_state()
    ...
    def manage_position(self) -> None:
        ...
        try:
            ...
            # Check for accidental exit conditions BEFORE managing trailing stops
            self.accidental_exit()
            
            # If position was reset by accidental_exit, return early
            if self.position == 0:
                return
