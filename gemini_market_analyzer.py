    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            ...
            try:
                ...
                logger.warning(f"Failed to initialize gemini-2.5-pro: {e}, trying gemini-2.5-flash...")
                try:
...
"""
    ...
    def analyze_multiple_consolidations(self, analyses: list) -> Dict[str, Tuple[bool, str]]:
        """
        Analyze multiple market consolidations in a single API call to save rate limits.
        
        Args:
            analyses: List of dicts with keys: {'df': DataFrame, 'symbol': str, 'context': Optional[str]}
            
        Returns:
            Dict mapping symbol to (is_consolidating, reasoning) tuple
        """
        if self.client is None:
            logger.error("Gemini client not initialized. Cannot analyze consolidation.")
            return {analysis['symbol']: (False, "Gemini client not initialized") for analysis in analyses}
        
        try:
            # Prepare market summaries for all symbols
            all_summaries = []
            symbols = []
            ...
            for analysis in analyses:
                ...
                context = analysis.get('context', None)
            ...
            # Create a combined prompt for all symbols
            market_data_section = "\n\n".join([
                f"=== {s['symbol']} ===\n{s['summary']}"
...
Your task is to analyze market data for MULTIPLE symbols and determine if each market is currently in a CONSOLIDATING/CHOPPY state or in a TRENDING state.
...
Market Data Provided for ALL SYMBOLS:
{market_data_section}

Based on ALL the above factors, determine if EACH market is CONSOLIDATING or TRENDING.
...
Respond in the following JSON format for EACH symbol:
{{
    "symbol1": {{
...
Only respond with the JSON object, no additional text."""
            ...
            for symbol in symbols:
                if symbol in result:
                    ...
                    results[symbol] = (
                        bool(symbol_result.get('is_consolidating', False)),
                        symbol_result.get('reasoning', 'No reasoning provided')
                    )
                else:
                    # Fallback if symbol not in response
                    results[symbol] = (False, 'Symbol not found in response')
            
            logger.info(f"Gemini batch analysis completed for {len(symbols)} symbols")
            
            return results
            
        except Exception as e:
            logger.error(f"Error in Gemini batch analysis: {e}", exc_info=True)
            # Return safe defaults for all symbols
            return {analysis['symbol']: (False, f"Error in analysis: {str(e)}") for analysis in analyses}
